...
def _validate_verse_ids(db: Session, start_verse_id: int, end_verse_id: int) -> None:
    ...
    start_exists = db.scalar(select(BibleVerses.verse_id).where(BibleVerses.verse_id == start_verse_id))
    end_exists = db.scalar(select(BibleVerses.verse_id).where(BibleVerses.verse_id == end_verse_id))
    if start_exists is None or end_exists is None:
        raise HTTPException(status_code=400, detail="Invalid start_verse_id or end_verse_id.")


def list_widgets(db: Session) -> list[BibleWidget]:
    """List widget entries ordered by weight desc then reference."""
    rows = db.scalars(_widget_query().order_by(BibleWidgetVerses.weight.desc(), BibleWidgetVerses.ref.asc())).all()
...
def create_widget(db: Session, payload: BibleWidget) -> BibleWidget:
    ...
    if payload.start_verse_id is None or payload.end_verse_id is None:
        raise HTTPException(status_code=400, detail="start_verse_id and end_verse_id are required.")
    if not payload.translation or not payload.ref or not payload.display_text:
        raise HTTPException(status_code=400, detail="translation, ref, and display_text are required.")
...
def patch_widget(db: Session, widget_id: int, payload: PatchedBibleWidget) -> BibleWidget:
