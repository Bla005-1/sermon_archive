...
@router.post("/", response_model=Sermon, status_code=status.HTTP_201_CREATED, operation_id="sermons_create")
...
def sermons_retrieve(
    sermon_id: int = Path(..., description="A unique integer value identifying this sermon."),
    db: Session = Depends(get_db),
) -> Sermon:
...
@router.patch("/{sermon_id}/", response_model=Sermon, operation_id="sermons_partial_update")
...
@router.delete("/{sermon_id}/", status_code=status.HTTP_204_NO_CONTENT, operation_id="sermons_destroy")
def sermons_destroy(
    sermon_id: int = Path(..., description="A unique integer value identifying this sermon."),
    db: Session = Depends(get_db),
) -> None:
...
@router.get("/{sermon_id}/attachments/", response_model=list[Attachment], operation_id="sermons_attachments_list")
...
def sermons_attachments_create(
    ...
    return attachment_service.create_sermon_attachment(db=db, sermon_id=sermon_id, file=file)


@router.get("/{sermon_id}/passages/", response_model=list[SermonPassage], operation_id="sermons_passages_list")
...
def sermons_passages_create(
    ...
    return sermons_service.create_sermon_passage(db=db, sermon_id=sermon_id, payload=payload)


@router.get("/{sermon_id}/passages/{id}/", response_model=SermonPassage, operation_id="sermons_passages_retrieve_2")
...
@router.put("/{sermon_id}/passages/{id}/", response_model=SermonPassage, operation_id="sermons_passages_update_2")
...
@router.patch("/{sermon_id}/passages/{id}/", response_model=SermonPassage, operation_id="sermons_passages_partial_update_2")
...
@router.delete("/{sermon_id}/passages/{id}/", status_code=status.HTTP_204_NO_CONTENT, operation_id="sermons_passages_destroy_2")
...
@router.get("/suggestions/", response_model=SermonSuggestionsResponse, operation_id="sermons_suggestions_retrieve")
def sermons_suggestions_retrieve(db: Session = Depends(get_db)) -> SermonSuggestionsResponse:
